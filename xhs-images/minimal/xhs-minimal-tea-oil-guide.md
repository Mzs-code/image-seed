|   |   |   |
|:---:|:---:|:---:|
| ![preview](./xhs-minimal-tea-oil-guide-0.jpeg) | ![preview](./xhs-minimal-tea-oil-guide-1.jpeg) | ![preview](./xhs-minimal-tea-oil-guide-5.jpeg) |

# 东方编辑美学课件 · 茶油选购指南(10 张系列)

> 这是一份**多变体共享 prompt**:同一份 prompt 一次性生成了 NO.01 - NO.10 共 10 张图,本 sidecar 被 `xhs-minimal-tea-oil-guide-0.jpeg` ~ `-9.jpeg` 共用。
>
> 主题:如何判定一瓶好茶油 · 用途:PPT / 课件 · 比例:3:4
...
文字设计是画面的核心:让中文、数字、英文和注释各自拥有不同尺度与语气,最大字负责视觉重量,中等字负责章节感,小字负责知识密度,英文或拼音只作为节奏性的细标。可以使用竖排与横排混合、字距拉开、局部旋转的极小标签、灰底小章、二维码式信息块、日期或编号,但都要服务于秩序,不要堆砌。最终画面应像一张可以被反复阅读的文化海报,也能自然转化为 PPT 封面、报告首页、信息图、排行榜、产品卡、人物专题或数据页:内容越复杂,留白越要坚定;信息越重要,强调色越要精确;主体越具象,周围越要轻。现在把这种美学用于我的实际内容,让它适合我给出的主题、文字、数据、物件或页面用途。

本次主题:如何判定一瓶好茶油
用途:ppt / 课件,请生成不低于 10 张图片
比例 3:4
注意是不是要你一张图片集合所有图片,是逐张生成。
```
