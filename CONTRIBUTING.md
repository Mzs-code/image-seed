...
6. **画廊聚簇**:共享同一份 sidecar(被 ≥2 张图引用)的图,脚本会在画廊里把它们从字母序拉到一起**连续相邻**(不折叠 —— 它们主题各异,每张各占一格、各带 📝),整簇出现在簇首位成员的位置。所以模板批次在画廊里一眼成组,无需手动排序
...
5. **同 trunk 系列折叠**:同一 trunk(去 `-NN`)且 ≥2 张的连续图(同 prompt/同故事的多帧,见 [多图共享 prompt](#多图共享-prompt)),在画廊里折叠成**单个 tile** —— 封面=首帧 + 左上「⧉ N」张数角标 + 堆叠投影;其余帧隐藏但共享 `data-gallery=<trunk>`,点封面开灯箱顺序翻看全组,每帧灯箱说明取该帧元数据主体(`data-title` 为「i / N」)。所以变体组/系列组**只占 1 格**,你正常按帧命名 `-01/-02…` 即可,无需为展示做任何特殊处理。**例外见 6:共享 `-template.md` 的组不折叠**
6. **模板共享聚簇**:Prompt 列指向**同一份 `-template.md` sidecar**(即「不同主题共用一份 prompt 模板」,如知识图解 6 张、节气海报 4 张)的图,**不折叠**(主题各异、各占一格、各带 📝),改从字母序里**拉到一起连续相邻**,整簇出现在簇首位成员的位置 —— 模板批次在画廊里一眼成组。判定:**sidecar 以 `-template.md` 结尾 = 模板共享 = 聚簇不折叠**,即便命名成 `-01..-04`。真·系列(同一作品多帧/多页,sidecar 非 `-template`)不受影响,仍按 5 折叠
